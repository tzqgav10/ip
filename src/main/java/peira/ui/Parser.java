...
public class Parser {
    ...
    private static Command parseDeadline(String input) throws PeiraExceptions {
        ...
        if (input.trim().isEmpty()) {
            throw new PeiraExceptions(INDENT + "Hey! What do you need to do? When must you complete it?");
        }
        int byIndex = -1;
        byIndex = input.indexOf(" /by ");
        // check if /by is included in the command
        if (byIndex == -1) {
            throw new PeiraExceptions(INDENT + "So... when's the deadline?");
        }
    ...
    private static Command parseEvent(String input) throws PeiraExceptions {
        if (input.trim().isEmpty()) {
            throw new PeiraExceptions(INDENT + "So... what event are you going to?");
        }
        int fromIndex = -1;
        fromIndex = input.indexOf(" /from ");
        if (fromIndex == -1) {
            throw new PeiraExceptions(INDENT + "So... when does the event start?");
        }
        String description = input.substring(0, fromIndex);
        int toIndex = -1;
        toIndex = input.indexOf(" /to ");
        if (toIndex == -1) {
            throw new PeiraExceptions(INDENT + "So... when does the event end?");
        }
